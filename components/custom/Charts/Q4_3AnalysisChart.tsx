...
interface Q4_3AnalysisChartProps {
    q4_3Data: Q4_3Data[];
}
...
export default function Q4_3AnalysisChart({ q4_3Data }: Q4_3AnalysisChartProps) {
    const chartData = q4_3Data
        .map((item, index) => ({
            campania: item.campania,
            averageQ4_3: item.averageQ4_3 ?? 0,
            totalResponses: item.totalResponses,
            color: colorPalette[index % colorPalette.length], // Assign color from palette
        }))
        .sort((a, b) => b.averageQ4_3 - a.averageQ4_3);

    const maxAverage = Math.max(...chartData.map(item => item.averageQ4_3));
                                ...
                                if (active && payload && payload.length > 0) {
                                    ...
                                    return (
                                        <div className="p-2 border rounded shadow bg-background text-foreground">
                                            ...
                                            <p>Medie: {data.averageQ4_3.toFixed(2)}</p>
