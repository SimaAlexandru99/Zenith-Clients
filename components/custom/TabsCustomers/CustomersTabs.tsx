...
const databaseInfo = {
  UT_database: {
    ...
    shortTitle: "UT",
    description: "Date despre clienții care au vizitat agențiile CEC Bank și feedback-ul lor despre serviciile oferite.",
    icon: UserCheck,
    metrics: [
  ...
  CC_database: {
    ...
    shortTitle: "CC",
    description: "Date despre clienții care au interacționat cu Call Center-ul și experiența lor cu serviciile telefonice.",
    icon: HeadphonesIcon,
    metrics: [
      ...
      <Tabs defaultValue="UT_database" onValueChange={(value) => setSelectedDb(value as DatabaseType)}>
        <TabsList className="h-10 mb-4">
          <TabsTrigger 
            ...
            aria-label="Select UT Customer Database"
          >
          ...
          <TabsTrigger 
            value="CC_database" 
            className="flex items-center gap-2"
            aria-label="Select CC Customer Database"
          >
